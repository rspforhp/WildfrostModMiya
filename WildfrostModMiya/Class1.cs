...
public static class JSONParser
{
    ...
    internal static object ParseValue(Type type, string json)
    {
                ...
                if (json[i] == '\\' && i + 1 < json.Length - 1)
                {
                    ...
                    if (json[i + 1] == 'u' && i + 5 < json.Length - 1)
                    {
                        ...
                        if (uint.TryParse(json.Substring(i + 2, 4),
                                NumberStyles.AllowHexSpecifier, null, out c))
                        {
        ...
        {
            var result = Convert.ChangeType(json, type.GetGenericArguments().First(),
        ...
        {
            decimal result;
            decimal.TryParse(json, NumberStyles.Float,
        ...
        {
            DateTime result;
            DateTime.TryParse(json.Replace("\"", ""), CultureInfo.InvariantCulture,
    ...
    private static object ParseAnonymousValue(string json)
    {
        ...
        if (char.IsDigit(json[0]) || json[0] == '-')
        {
            ...
            {
                double result;
                double.TryParse(json, NumberStyles.Float,
                    CultureInfo.InvariantCulture, out result);
                return result;
            }
            else
...
{
    public static string VanillaEffectName(this CardBuilder.VanillaEffects effect)
    {
        return CardBuilder.VanillaEffectsNamesLookUp[effect];
    }

    public static string VanillaTraitName(this CardBuilder.VanillaTraits traits)
    {
        return CardBuilder.VanillaTraitsNamesLookUp[traits];
    }
}
...
public class CardBuilder
{
    ...
    internal static readonly Dictionary<VanillaEffects, string>
    ...
    public enum VanillaPoolsNames
    {
        ...
        /// <summary>
        ///     General Pools are shared between all classes, add to pools of any class, once, it doesnt matter which one
        /// </summary>
        GeneralItemPool,
        GeneralUnitPool,
        GeneralCharmPool,
    ...
    public CardBuilder AddCardToClassAndPool(string className, VanillaPoolsNames poolName, int weight = 1,
        int value = 1)
    {
        return AddCardToClassAndPool(className, poolName.ToString().Replace("VanillaPoolsNames.", ""), weight, value);
    }


    public CardBuilder AddCardToClassAndPool(VanillaClassesNames className, VanillaPoolsNames poolName, int weight = 1,
        int value = 1)
    {
        return AddCardToClassAndPool(className.ToString().Replace("VanillaClassesNames.", ""),
            poolName.ToString().Replace("VanillaPoolsNames.", ""), weight, value);
    }

    public CardBuilder AddCardToClassAndPool(VanillaClassesNames className, string poolName, int weight = 1,
        int value = 1)
    {
        return AddCardToClassAndPool(className.ToString().Replace("VanillaClassesNames.", ""), poolName, weight, value);
    }
    ...
    public CardBuilder AddCardToClassAndPool(string className, string poolName, int weight = 1, int value = 100)
    {
        ...
        foreach (var pool in AddressableLoader.groups["ClassData"].lookup[className].Cast<ClassData>().rewardPools)
            ...
            {
                for (var i = 0; i < weight; i++) pool.list.Add(data);
                // pool.list.Clear();//DEEBUG
                Mod.Instance.LoggerInstance.Msg(
                    $"Added card {data.title} to pool {poolName} of class {className} {weight} times!");
                break;
            }
    ...
    public CardBuilder AddStartWithEffects(params EffectData[] Effects)
    {
        ...
        var listTest = IArrayExt.ToList(data.startWithEffects);
        ...
        data.startWithEffects =
            new Il2CppReferenceArray<CardData.StatusEffectStacks>(
                listTest.ToArray());
        return this;
    }
    ...
    {
        var dictionary = AddressableLoader.groups["StatusEffectData"].lookup;
        var listTest = IArrayExt.ToList(data.attackEffects);
...
public static class WildFrostAPI
{
    ...
    private static IEnumerator AddAllCardsCoroutine(List<CardBuilder> cardBuilders)
    {
        ...
        {
            AttackEffects = new[]
                { new CardBuilder.EffectData(CardBuilder.VanillaEffects.Frost.VanillaEffectName()) }
        }));
        foreach (var jsonFile in Directory.EnumerateFiles(jsonCardsLoc, "*.json"))
        {
            Mod.Instance.LoggerInstance.Warning("Will add " + jsonFile);
            var data = File.ReadAllText(jsonFile).FromJson<Mod.JSONCardData>();
    ...
    {
        MelonCoroutines.Start(AddAllCardsCoroutine(cardBuilders));
    }
}
...
public class Mod : MelonMod
{
    ...
    private void DoUnecryptedSaveLogicUpdate()
    {
    ...
    public override void OnInitializeMelon()
    {
        ...
        WildFrostAPI.AddCards += delegate(List<CardBuilder> list)
        {
            list.Add(new CardBuilder()
                .SetTitle("NewCard")
                .SetStats(damage: 1)
                .SetIsItem(true)
                .AddAttackEffects(new CardBuilder.EffectData("Demonize"))
                .AddTraits(new CardBuilder.TraitData("Noomlin"))
                .AddCardToClassAndPool(CardBuilder.VanillaClassesNames.Basic,
                    CardBuilder.VanillaPoolsNames.GeneralItemPool, 100)
            );
            ...
            list.Add(new CardBuilder()
                ...
                .SetStats(5, 1, 3)
                .AddTraits(new CardBuilder.TraitData("Smackback"))
                .AddTraits(new CardBuilder.TraitData("Barrage"))
                .AddAttackEffects(
                    new CardBuilder.EffectData("Frost", 5))
            );
            */
    ...
    public override void OnUpdate()
    {
        if (AddressableLoader.IsGroupLoaded("CardData") && AddressableLoader.IsGroupLoaded("CardType") &&
            ...
            if (!AddressableLoader.groups["CardData"].list.Contains(debug))
            {
                ...
                var builders = new List<CardBuilder>();
                WildFrostAPI.InvokeEvents(ref builders);
                WildFrostAPI.AddAllCards(builders);
            }
    ...
    private class IsUnlocked1
    {
        [HarmonyPostfix]
        private static void Postfix(MetaprogressionSystem __instance, ref bool __result)
        {
            if (UnlockAll)
    ...
    [HarmonyPatch(typeof(MetaprogressionSystem), nameof(MetaprogressionSystem.IsUnlocked), typeof(UnlockData),
        typeof(Il2CppSystem.Collections.Generic.List<string>))]
    private class IsUnlocked2
    {
        [HarmonyPostfix]
        private static void Postfix(MetaprogressionSystem __instance, ref bool __result)
        {
